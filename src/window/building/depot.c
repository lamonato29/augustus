...
void window_building_draw_depot_foreground(building_info_context *c)
{
    ...
    if (src->storage_id) {
        text_draw_centered((const uint8_t*)building_get_display_name(src),
    ...
    if (dst->storage_id) {
        text_draw_centered((const uint8_t*)building_get_display_name(dst),
...
void window_building_draw_depot_select_source_destination(building_info_context *c)
{
    ...
    for (int i = 0; i < storage_array_size && drawn_rows < MAX_VISIBLE_ROWS; i++) {
        ...
        if (storage->in_use && max_storable > 0) {
            ...
            text_draw_centered((const uint8_t*)building_get_display_name(bld),
                c->x_offset + 18 + BLOCK_SIZE * 2,
